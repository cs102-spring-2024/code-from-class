...
public:
    ...
    int sum();                   // returns the sum of all elements in the list
private:
    Node * _head;
    int _size;
...
/**
 * @brief Deletes all nodes in linked list starting at curr
 * 
 * @param curr 
 */
...
void LinkedList::delete_node(int i) {
    ...
    if (i == 0) {
        ...
        Node * before = get_node(i - 1);
        Node * to_delete = before->_next;
        before->_next = to_delete->_next;
        delete to_delete;
    }
}
...
int LinkedList::sum() {
    return sum_recursive(_head);
}

int LinkedList::sum_recursive(Node * curr) {
    if (curr == nullptr)
        return 0;

    return curr->_data + sum_recursive(curr->_next);
}
...
int main() {
    ...
    cout << "sume of smaller: " << smaller.sum() << endl;

    smaller.delete_node(0);
    smaller.display();
